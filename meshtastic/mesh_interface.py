...
from meshtastic.protobuf import admin_pb2, mesh_pb2, portnums_pb2, telemetry_pb2, xmodem_pb2
...
class MeshInterface:  # pylint: disable=R0902
    ...
    def _delete_remote_file(self, remote_path: str) -> None:
        if not remote_path:
            return
        try:
            self.localNode.ensureSessionKey()
        except Exception:
        ...
        try:
            ...
            self.localNode._sendAdmin(admin_msg, wantResponse=False)
        ...
        with self._xmodem_lock:
            ...
            if mode == "download":
                if control in (
                    ...
                    logger.error(
                        "Device cancelled XMODEM transfer for %s", state.get("path")
                    )
                    self._complete_xmodem_locked(
                ...
                if control == xmodem_pb2.XModem.Control.ACK:
                    ...
                    if awaiting in (None, "start"):
                        ...
                        if retries > state.get("max_retries", 5):
                            self._complete_xmodem_locked(
                                False, "Too many NAKs received during upload."
                            )
                            control_to_send = xmodem_pb2.XModem.Control.CAN
                        else:
                        ...
                        if retries > state.get("max_retries", 5):
                            self._complete_xmodem_locked(
                                False, "Timeout while finalizing upload."
                            )
                            control_to_send = xmodem_pb2.XModem.Control.CAN
                        else:
                            control_to_send = xmodem_pb2.XModem.Control.EOT
                    ...
                    self._complete_xmodem_locked(
                        False, "Device cancelled the XMODEM upload."
                    )
                else:
    ...
    def upload_file(
        ...
        *,
        ...
        if not host_path.is_file():
            raise MeshInterface.MeshInterfaceError(
                f"Host source '{host_src}' is not a file."
            )
